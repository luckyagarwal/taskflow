...
- **Parsing path.** Capture first tries the regex parser; on low confidence it falls back to a Claude
  Haiku call that returns a structured task (title, dueOffset, time, priority, labels).
...
- **Secrets.** WhatsApp token, phone-number id, verify token, app secret, allowed number, and the
  Anthropic API key are stored as Cloudflare secrets.
