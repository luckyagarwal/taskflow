...
export function DueBadge({ offset, time, small, startOffset, startTime, duration }) {
  const labelText = H.dateRangeLabel(startOffset, offset, time, startTime, duration);
      ...
      {showProject !== 'inDate' && (
        <div style={{ position: 'relative' }} onClick={(e) => e.stopPropagation()}>
          <button onClick={() => setMenu(menu === 'due' ? null : 'due')} style={{ border: 'none', background: 'transparent', padding: 0, cursor: 'pointer', display: 'flex' }}>
            <DueBadge offset={task.dueOffset} startOffset={task.startOffset} time={task.time} startTime={task.startTime} duration={task.duration} />
          </button>
          {menu === 'due' && (
            <DateSelectorModal
              task={task}
